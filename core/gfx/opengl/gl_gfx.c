...
static void set_backend_ptrs(renderer_t* r)
{
  ...
  r->backend.buffer_object_create = gl_buf_obj_create;
  r->backend.buffer_object_destroy = gl_buf_obj_destroy;
  r->backend.buffer_object_bind = gl_buf_obj_bind;
  r->backend.buffer_object_unbind = gl_buf_obj_unbind;
  r->backend.buffer_object_set = gl_buf_obj_set_dat;
  r->backend.vertex_array_create = gl_vert_arr_create;
  r->backend.vertex_array_destroy = gl_vert_arr_destroy;
  r->backend.vertex_array_bind = gl_vert_arr_bind;
  r->backend.vertex_array_draw = gl_vert_arr_draw;
  r->backend.vertex_array_draw_indexed = gl_vert_arr_draw_idx;
  r->backend.framebuffer_create = gl_framebuf_create;
  r->backend.framebuffer_destroy = gl_framebuf_destroy;
  r->backend.framebuffer_bind = gl_framebuf_bind;
  r->backend.framebuffer_resize = gl_framebuf_resize;
  r->backend.framebuffer_draw = gl_framebuf_draw;
